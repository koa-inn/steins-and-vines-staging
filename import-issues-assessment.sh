#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────
# Steins & Vines — Assessment Issues Importer (Testing + Docs)
# Adds 12 new findings from the testing strategy & documentation assessment.
#
# Prerequisites: gh CLI installed and authenticated (gh auth login)
# Usage: bash import-issues-assessment.sh [--dry-run] [--repo OWNER/REPO]
# ─────────────────────────────────────────────────────────────────
...
DRY_RUN=false
CSV_FILE="$(dirname "$0")/github-issues-assessment.csv"
DELAY=2
...
echo ""
echo "  Steins & Vines — Assessment Issues (Testing + Docs)"
echo "  Repo: $REPO"
echo ""

# Ensure new labels exist
LABEL_DEFS=(
    "source:testing-assessment:a8d8ea"
    "source:docs-assessment:b4e0a8"
    "type:docs:0075ca"
    "type:testing:fbca04"
)
