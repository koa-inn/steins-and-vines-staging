...
## v4.9 Requirements (Sheets → Postgres, Full Transition)

**Defined:** 2026-09-23. **Source:** `.planning/research/sheets-to-postgres-migration.md` §4 Stages 0–6 (2026-09-02) and `.planning/notes/sheets-to-postgres-data-conversion.md`. **Owner decision 2026-09-23:** all six stages committed up front; the research's §9 trigger gating is dropped (its Stages-0–2-only recommendation is recorded, not followed). Strict order 82 → 88. **Decisions:** production keeps a fire-and-forget sheet mirror for every migrated table indefinitely (hand-edits stop being authoritative); staging writes only to its own Postgres and never to the shared workbook; Schedule/Homepage go to committed repo content, never Postgres. **Cross-cutting (binding on every DB-xx):** money `numeric(10,2)`; per-row timestamp normalisation, reject-not-coerce; existing text IDs kept and sequence-backed; `''`→`NULL`; string booleans → `boolean`; `steps` → `jsonb`; `recipe_ingredients` never unique on `(recipe_id, item_id)`; `dual` ≥1 week with Sentry discrepancy logging before any flip; every flip has a written rollback; tests run against real Postgres, not mocks.

- [ ] **DB-01** (Phase 82): No browser code calls Apps Script directly — `js/admin.js` (55 sites) and `js/batch.js` route through the middleware; the 7 zero-caller actions are deleted; `updateGiftCardInvoice` and the `createBatch` dedup guard run under the lock; the four owner checks (tab formulas, Executions failures, Railway plan, live row counts) are answered.
- [ ] **DB-02** (Phase 83): Postgres provisioned in staging AND production with `lib/db.js` (pool/query/transaction), `node-pg-migrate` on deploy, `DATABASE_URL` validated, a Testcontainers Jest harness on CI, a per-store `sheets|dual|postgres` flag helper with the mirror hard-off on staging, and a reusable backfill pipeline (snapshot → normalise → rejects → scratch → promote).
- [ ] **DB-03** (Phase 84): Gift-card balance of record is Postgres — `gift_cards` + `gift_card_transactions` (`tx_ref UNIQUE`), redeem/reload atomic, all call sites flagged, backfilled to the cent, ≥1-week dual, flipped, kiosk balance-adjust control replaces sheet hand-edits; live kiosk sale/lookup/void verified.
- [ ] **DB-04** (Phase 85): Recipes + ingredients relational and transactional, `ingredient_id` sole key, IDs unchanged; list/detail/public API/pricing/admin editor flagged; Phase 73/79 pricing and unit behaviour proven identical by parity tests; dual then flip.
- [ ] **DB-05** (Phase 86): Vessels, FermSchedules, Config in Postgres with a Vessels admin screen and a Staff Access screen; `server_token` in Railway env only; security review on the allowlist path; schedule IDs sequence-backed.
- [ ] **DB-06** (Phase 87): Batches, BatchTasks, PlatoReadings, VesselHistory in Postgres via an approved design pass and a rehearsed maintenance-window cutover; `js/brewpad.js` unchanged (admin-proxy swap); public batch view and kiosk-sale batch creation intact; dashboard numbers match the pre-cutover snapshot.
- [ ] **DB-07** (Phase 88): Legacy tabs (Reservations/Holds/Kits + form trigger) deleted; Schedule/Homepage served from committed repo content; Railway backups verified by a restore drill plus scheduled `pg_dump` to Drive; `adminApi.gs` reduced to the mirror writer or removed; DATA-MODEL/APPS_SCRIPT/RUNBOOK/CLAUDE.md updated.

---
*Requirements defined: 2026-07-03 (v4.5); 2026-07-10 (v4.6 analytics); 2026-07-15 (v4.7 post-review polish); 2026-07-24 (v4.8 bookkeeping & inventory integrity); 2026-09-23 (v4.9 Sheets → Postgres)*
*Last updated: 2026-09-23 — added v4.9 Sheets → Postgres, Full Transition (DB-01..07, Phases 82-88).*
