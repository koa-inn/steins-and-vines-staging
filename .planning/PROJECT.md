...
## Planned Milestone: v4.9 Sheets → Postgres (Full Transition)

**Goal:** Replace the Google Sheets operational store behind `apps-script/adminApi.gs` with Postgres on Railway (provisioned in both environments), migrated one sheet-group at a time behind a per-store `sheets` | `dual` | `postgres` flag with an instant, lossless rollback, until Apps Script is no longer a system of record for anything.

**Target features (Phases 82–88, strict order, one per research stage):**
- **DB-01 Store-agnostic prerequisites** — `js/admin.js` (55 direct calls) and `js/batch.js` routed through the middleware seam; 7 zero-caller Apps Script actions deleted; the two lock holes closed; owner checks (tab formulas, Executions failures, Railway plan, live row counts).
- **DB-02 Postgres infrastructure** — Railway Postgres in staging AND production, `lib/db.js`, `node-pg-migrate`, Testcontainers Jest harness, per-store flag helper, reusable backfill pipeline.
- **DB-03 GiftCards** — atomic redeem/reload on an append-only ledger; dual window; kiosk balance-adjust control.
- **DB-04 Recipes + RecipeIngredients** — relational, transactional, `ingredient_id` sole key; pricing/unit parity proven.
- **DB-05 Vessels + FermSchedules + Config** — plus the missing Vessels and Staff Access admin screens; `server_token` to env.
- **DB-06 Batches + BatchTasks + PlatoReadings + VesselHistory** — design pass, then a rehearsed maintenance-window cutover; BrewPad unchanged via the admin-proxy seam.
- **DB-07 Retire the legacy sheets** — Reservations/Holds/Kits deleted; Schedule/Homepage to committed repo content; Postgres-native backups with a restore drill; docs updated.

**Key context:** Source of truth is `.planning/research/sheets-to-postgres-migration.md` (2026-09-02) and `.planning/notes/sheets-to-postgres-data-conversion.md`. The research recommended Stages 0–2 only and gating the rest on measured triggers; **the owner chose the full transition on 2026-09-23** and that gating is dropped. Owner decisions the same day: production keeps a fire-and-forget sheet mirror for every migrated table indefinitely (hand-edits stop being authoritative); staging never mirrors to the shared workbook; Schedule/Homepage never go to Postgres. Honest estimate: 25–40 developer-days. Additive — nothing renumbered; phases continue from 81.
...
|----------|-----------|---------|
| Keep Google Sheets as batch backend | Already working, staff familiar, Apps Script API adequate | ✓ Good for v1–v4.8; superseded by v4.9 (below) |
| Migrate the whole operational store to Postgres on Railway, all six stages (v4.9, owner decision 2026-09-23) | Sheets has no transactions (gift-card redeem is 4 non-atomic writes), an inconsistent global lock, unindexed scans, an un-CI'd Apps Script backend, and one workbook shared by staging and prod. Research recommended GiftCards-only; owner chose the full transition. Mitigation for the lost "open the sheet and fix it" affordance: a permanent fire-and-forget mirror | — Pending |
